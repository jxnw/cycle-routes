...
I would like to thank my supervisor, Dr Michael Young, for his invaluable help and guidance throughout the project. This dissertation would not have been possible without his helpful and constructive feedback.

My gratitude extends to my partner, Declan, for his unconditional support in this journey.

\chapter*{Declaration}
I declare that the material submitted for assessment is my own work except where credit is explicitly given to others by citation or acknowledgement. This work was performed during the current academic year except where otherwise stated.

The main text of this project report is 11,031 words long, including project specification and plan.
...
\begin{figure}[ht]
    \centering
    \includesvg[width=0.8\textwidth]{diss_images/impl/networkx.svg}
...
\begin{figure}[ht]
    \centering
    \adjustbox{trim=2cm 1.2cm 2cm 1.5cm,clip} {
        \includesvg[width=\textwidth]{diss_images/impl/graph_layout.svg}
    }
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[ht]{\textwidth}
        \centering
        \adjustbox{trim=5cm 4cm 3cm 2cm,clip} {
            \includesvg[width=\textwidth]{diss_images/impl/close_before.svg}
        }
    ...
    \begin{subfigure}[ht]{\textwidth}
        \centering
        \adjustbox{trim=5cm 4cm 3cm 2cm,clip} {
            \includesvg[width=\textwidth]{diss_images/impl/close_after.svg}
        }
...
\begin{figure}[ht]
    \centering
    \includesvg[width=0.8\textwidth]{diss_images/impl/components.svg}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}[ht]{0.65\textwidth}
        \centering
        \includegraphics[width=\textwidth,trim={1.2cm 1.3cm 1.2cm 1.2cm},clip]{diss_images/eval/cycle_friendly_old.png}
        \caption{Before the configuration was modified}
        \label{fig:cycle friendly graph old}
    \end{subfigure}
    \hfill
    \begin{subfigure}[ht]{0.62\textwidth}
        \centering
...
The cycle path additions suggested by our system are highlighted in yellow in Figure~\ref{fig:path st andrews}. As might be expected from the initial conditions given to each approach, strategy A chose the shortest connection along the east edge, utilizing North Castle Street. Strategy B chose a route through the centre of St Andrews, strategy C connected the University Area through to the northern shore and strategy D produced a similar path to strategy A, except it used nearby pre-existing cycle paths where it could. 

These results highlight the trade-offs inherent to town planning. Strategy A might be the cheapest, but it won't benefit a large number of people, while strategies B and C each favour different groups. It would benefit students the most to improve cycling connections from the university to the centre, whereas normal residents would greatly benefit from a less dangerous approach into town. Strategy D showed how the idea produced by strategy A could be implemented more practically, considering existing infrastructure. Our system can give town planners concrete suggestions of routes they could plan, and they can select different strategies to use based on their priorities for improvements.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|}
        \hline
        Graph & \makecell{Average \\shortest \\path length (km)} & \makecell{Percentage \\change \\w.r.t. unmodified \\graph} & \makecell{Length of \\suggested \\path (km)}\\ 
        \hline
        Unmodified & 3.928 & N/A & N/A\\
        Strategy A & 3.588 & $-8.656\%$ & 0.139\\
        Strategy B & 3.587 & $-8.681\%$ & 0.221\\
        Strategy C & 3.597 & $-8.427\%$ & 0.855\\
        Strategy D & 3.587 & $-8.681\%$ & 0.107\\
        \hline
    \end{tabular}
...
Table~\ref{tab:avg st andrews} shows the value of this metric for the unmodified cycle-friendly graph as well as for the graph modified by each of the strategies. All the strategies decreased the average shortest path length by a similar amount. Strategies A, B and D produced similar routes, and their values are almost identical. Strategy C produced a different route which was much longer. Although C started in the centre of the component, the route itself is quite long, so it did not have as strong an effect on decreasing the average shortest path length as the other strategies.

Considering the suggested path length, we can see that strategy D had the lowest value of 0.107. This has to be the case by definition, because it uses the shortest absolute shortest path and is the only route that will consider existing cycling infrastructure to be zero cost. Since the path suggested by strategy D is similar to the ones from A and B, it seems like adding this path would be the most value for money.

\section{Application to Other Areas}
In order to evaluate the scalability of our approach, we have applied the automated process to two other cities: Dundee in Scotland and Houten in the Netherlands. Dundee was chosen because it is nearby St Andrews, and it is much larger, so we might get multiple similarly sized regions rather than one big component. Houten was chosen because it has been the subject of urban planning designed to promote bicycle usage over driving.
...
The suggested paths are shown in Figure~\ref{fig:path dundee}. Our system suggests that the Ninewells Hospital area and the city centre should be the priority for connecting. This is in fact a project that Dundee City Council are undertaking~\cite{robertson:dundee}. The proposed routes from our system from strategies A, C and D are similar to those proposed by Dundee City Council. These suggested paths use the pre-existing cycle route along the waterfront and connect a small portion of Perth Road to this route. The council's proposal involve improving the cycling infrastructure along the whole of Perth Road, which would be more direct but also more costly. Strategy B is much more direct, but it goes through the Balgay Hill area, where there would be steep changes in elevation. Our system doesn't add extra costs when there are changes in elevation. If it did, then perhaps the suggested route from strategy B would go through the whole of Perth Road and be very close to the proposed route from the council. Note that the local centre of the red region is not in the city centre, where most of the nodes are located. This is because the red region is very thinly spread out.
...
\begin{table}[ht!]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        Strategy A & 26.301 & $-2.722\%$ & 2.136\\
        Strategy B & 26.272 & $-2.829\%$ & 3.175\\
        Strategy C & 26.280 & $-2.800\%$ & 3.509\\
        Strategy D & 26.207 & $-3.070\%$ & 0.628\\
        \hline
    \end{tabular}
...
As before, there was not much variance in the average shortest path length between strategies. We can see that strategy D had the lowest value, because it was connecting more than two cycle-friendly regions by utilising existing cycle paths in blue, which means it impacts the greatest number of nodes. Strategy D also had the lowest suggested path length due to usage of zero cost existing paths, even though superficially it suggests a similar path to strategies A and B. 
...
\begin{table}[ht!]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        Strategy A & 2.172 & $-2.426\%$ & 0.123\\
        Strategy B & 2.171 & $-2.471\%$ & 1.553\\
        Strategy C & 2.170 & $-2.516\%$ & 1.369\\
        Strategy D & 2.172 & $-2.426\%$ & 0.123\\
        \hline
    \end{tabular}
...
This time there was little variation in the scores. Strategies A and D suggested the same path, and strategies B and C suggested very similar paths. Since Houten already has very good cycling infrastructure, there are not many places that can be connected better, and this reflected in the similar paths of each strategy. The suggested paths from strategies run parallel to a street called \textit{Veerwagenpad}. This street has the OSM tag \texttt{cyclestreet=yes}, and it should be considered cycle-friendly. However, our system did not pick up this tag because of the terminology differences between the Dutch-speaking mappers and the English-speaking mappers: the \texttt{cyclestreet} tag did not appear when we checked the St Andrews tags. Most of the streets in Houten are labelled with \texttt{highway=cycleway} which did get recorded as having a high score.
...
\begin{enumerate}
    ...
    \item Currently, we produce undirected graphs. This does not reflect the nature of all road systems, even for cyclists. Therefore, we could extend the model to produce a directed graph output where one-way cycle paths are only considered in the direction they are in. Similarly, the graph layer would have to be extended so that the algorithms could run on a directed graph, and the suggestions could include modifying a one-way street to be a two-way street. We would also need to consider how we can tell the direction of the cycle paths from the OSM tags, and also whether there are differences between countries that drive on the left and those that drive on the right.
\end{enumerate}
...
\appendix
\chapter{README}\label{app:readme}
\begin{markdown}
